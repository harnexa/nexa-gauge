...
from ng_graph.nodes import report as report_node
            ...
            for outcome in runner.run_cases_iter(
                ...
                if outcome.result is not None:
                    ...
                    if case_report_dir:
                        ...
                        if report is None:
                            # For targets before the ``report`` node (e.g. ``chunk``),
                            # synthesize the same projection from final state.
                            report = report_node.aggregate(state=result.final_state)
                        _write_report_json(
                            report=report,
