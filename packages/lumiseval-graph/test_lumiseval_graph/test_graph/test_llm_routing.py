...
def test_node_grounding_uses_canonical_key_and_handles_missing_context(graph_module, monkeypatch) -> None:
    ...
    assert captured["resolved_node_name"] == "grounding"
    assert captured["resolved_overrides"] == llm_overrides
    assert captured["constructor_model"] == "resolved-grounding-model"
    assert captured["constructor_overrides"] == llm_overrides
    assert captured["enable_grounding"] is False
    assert captured["context_text"] == "Paris is in France."
    assert out["grounding_metrics"] == {"ok": True, "claims": 1}
